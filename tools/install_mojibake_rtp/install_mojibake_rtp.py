...
def install(project: Path, rtp_collection: str) -> bool:
    if not project.is_dir():
        raise ValueError(f"游戏目录不存在: {project}")

    rtp = _load_rtp_module()
    archive = Path(rtp_collection)
    if not archive.is_file():
        raise FileNotFoundError(f"缺少 RTP 资源库: {archive}")
    return rtp.install_rtp_files(str(project), ["2000fix"], collection_path=archive)


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--project", required=True)
    parser.add_argument("--rtp-collection", required=True)
    args = parser.parse_args()
    try:
        print("正在安装乱码版本的 RTP...", flush=True)
        if not install(Path(args.project), args.rtp_collection):
