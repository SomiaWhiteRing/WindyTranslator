...
def run_apply_base_dictionary(game_path: str, works_dir: str, world_dict_config: dict, broker: MessageBroker, **kwargs: object) -> None:
    """
    应用基础字典到游戏特定的生成字典。
    ...
    Args:
        ...
        broker (MessageBroker): 类型化消息代理。
        **kwargs 可能包含 'task_id_for_callback'，用于回调给特定的UI组件。
    """
    broker.status("正在应用基础字典...")
    log.info("开始应用基础字典流程...")
    ...
    try:
        ...
        if not os.path.exists(untranslated_json_path):
            broker.error(f"未找到原文 JSON 文件: {untranslated_json_path}，无法执行添加步骤。替换步骤仍可进行。")
        ...
        if not base_char_dict and not base_entity_dict:
            broker.log("基础字典为空或加载失败，跳过应用基础字典流程。")
            broker.status("应用基础字典已跳过 (无基础数据)")
        ...
        if os.path.exists(game_char_dict_path):
            try:
                ...
                broker.log(f"已加载游戏人物生成字典 '{char_gen_filename}': {len(game_char_data_list)} 条。使用表头: {game_char_headers}")
            except Exception as e:
                log.exception(f"加载游戏人物生成字典 {game_char_dict_path} 失败: {e}")
                broker.warning(f"加载游戏人物生成字典 '{char_gen_filename}' 失败: {e}。相关操作可能受影响。将使用默认表头。")
                game_char_headers = dictionary_manager.BASE_CHARACTER_HEADERS # 确保有默认表头
        else:
            broker.log(f"游戏人物生成字典 '{char_gen_filename}' 未找到，将使用默认表头并尝试处理。")
        ...
        if os.path.exists(game_entity_dict_path):
            try:
                ...
                broker.log(f"已加载游戏事物生成字典 '{entity_gen_filename}': {len(game_entity_data_list)} 条。使用表头: {game_entity_headers}")
            except Exception as e:
                log.exception(f"加载游戏事物生成字典 {game_entity_dict_path} 失败: {e}")
                broker.warning(f"加载游戏事物生成字典 '{entity_gen_filename}' 失败: {e}。相关操作可能受影响。将使用默认表头。")
                game_entity_headers = dictionary_manager.BASE_ENTITY_HEADERS
        else:
            broker.log(f"游戏事物生成字典 '{entity_gen_filename}' 未找到，将使用默认表头并尝试处理。")
            game_entity_headers = dictionary_manager.BASE_ENTITY_HEADERS
        ...
        broker.log(f"主译名替换完成，共替换 {replacements_made_count} 个条目的译名。")

        if old_to_new_translation_global_map:
            broker.log(f"开始在所有字段中查找并替换旧译名 (共 {len(old_to_new_translation_global_map)} 组替换)...")
            ...
            broker.log(f"字段内容替换完成，共更新 {fields_updated_count} 个字段。")
        else:
            broker.log("没有需要进行全局字段内容替换的旧译名。")
        ...
        if not os.path.exists(untranslated_json_path):
            broker.warning(f"原文 JSON 文件 ({os.path.basename(untranslated_json_path)}) 不存在，无法执行添加新术语的步骤。")
        else:
            ...
            broker.log(f"添加步骤完成，新增人物条目: {added_char_count}，新增事物条目: {added_entity_count}。")

        # --- 5. 保存修改后的游戏特定字典 ---
        broker.log("正在保存更新后的游戏特定字典...")
        ...
        if os.path.exists(game_char_dict_path) or game_char_data_list:
            try:
                ...
                broker.log(f"游戏人物字典 '{char_gen_filename}' 已更新并保存。", "success")
            except Exception as e:
                log.exception(f"保存更新后的游戏人物字典 '{char_gen_filename}' 失败: {e}")
                broker.error(f"保存游戏人物字典 '{char_gen_filename}' 失败: {e}")
        else:
            char_save_success = True
            log.info(f"游戏人物字典文件 '{char_gen_filename}' 不存在且无数据可写，跳过保存。")
            broker.log(f"跳过保存游戏人物字典 '{char_gen_filename}' (文件不存在且无数据)。")
        ...
        if os.path.exists(game_entity_dict_path) or game_entity_data_list:
            try:
                ...
                broker.log(f"游戏事物字典 '{entity_gen_filename}' 已更新并保存。", "success")
            except Exception as e:
                log.exception(f"保存更新后的游戏事物字典 '{entity_gen_filename}' 失败: {e}")
                broker.error(f"保存游戏事物字典 '{entity_gen_filename}' 失败: {e}")
        else:
            entity_save_success = True
            log.info(f"游戏事物字典文件 '{entity_gen_filename}' 不存在且无数据可写，跳过保存。")
            broker.log(f"跳过保存游戏事物字典 '{entity_gen_filename}' (文件不存在且无数据)。")
        ...
        if char_save_success and entity_save_success:
            ...
            broker.success(callback_message_content)
            broker.status("应用基础字典完成")
        else:
            callback_task_successful = False
            callback_message_content = "应用基础字典过程中保存文件失败，请检查日志。"
            broker.error(callback_message_content)
            broker.status("应用基础字典失败 (保存错误)")

    except Exception as e:
        log.exception("应用基础字典任务执行期间发生意外错误。")
        broker.error(f"应用基础字典过程中发生严重错误: {e}")
        broker.status("应用基础字典失败")
        callback_task_successful = False
        callback_message_content = f"应用基础字典过程中发生严重错误: {e}"

    finally:
        # 任务结束时，发送带 task_id 的 "done" 信号（用于编辑器回调）
        if task_id_for_callback:
            log.debug(f"任务完成，发送带回调ID '{task_id_for_callback}' 的 done 信号。成功: {callback_task_successful}")
            broker.done(task_id=str(task_id_for_callback), success=callback_task_successful, message=callback_message_content)
        else:
